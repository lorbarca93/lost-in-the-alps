...
<head>
    ...
    <!-- Leaflet MarkerCluster CSS with SRI -->
    ...
    <!-- Leaflet MarkerCluster JS with SRI -->
    ...
    <!-- Fuse.js for fuzzy search with SRI -->
    <script src="https://cdn.jsdelivr.net/npm/fuse.js@6.6.2"
            integrity="sha384-zPE55eyESN+FxCWGEnlNxGyAPJud6IZ6TtJmXb56OFRGhxZPN4akj9rjA3gw5Qqa" crossorigin="anonymous"></script>
    <style>
        * {{ 
            margin: 0; 
            padding: 0; 
...
<body>
    ...
    <div class="sidebar">
        ...
        <div class="sidebar-content">
            ...
            <div class="filter-section favorites-section">
                ...
                <div class="stats-mini">
                    <div>
                        <div style="font-size: 32px; font-weight: 700; color: #f59e0b;" id="favorites-count">0</div>
                        <div style="font-size: 11px; opacity: 0.9; color: #d97706;">Saved Huts</div>
                    </div>
                </div>
                <div style="display: flex; flex-direction: column; gap: 10px; margin-top: 14px;">
                    <button class="btn" style="background: linear-gradient(135deg, #f59e0b 0%, #d97706 100%); color: white;" onclick="showFavoritesOnly()">
                        ⭐ Show My Favorites
                    </button>
                    <button class="btn btn-secondary" onclick="showAllHutsReset()">
                        🗺️ Show All Huts
                    </button>
                    <button class="btn btn-secondary" onclick="FavoritesManager.exportJSON()">
                        📥 Download Favorites
                    </button>
                    <button class="btn btn-secondary" onclick="FavoritesManager.importJSON()">
                        📤 Upload Favorites
                    </button>
                    <button class="btn btn-secondary" onclick="FavoritesManager.exportGPX()">
                        🧭 Export to GPX
                    </button>
                </div>
                <div style="margin-top: 12px; padding: 10px; background: rgba(245, 158, 11, 0.1); border-radius: 8px; font-size: 11px; color: #92400e; line-height: 1.5;">
                    <strong>💡 Tip:</strong> Download your favorites as backup! You can re-upload them if your browser data gets cleared.
                </div>
            </div>
    ...
    <script>
        ...
        huts.forEach(function(hut) {{
            ...
            // Simplified, larger markers - easy to click!
            var isMobile = window.innerWidth <= 768;
            var baseRadius = isMobile ? 12 : 7;  // Larger for easier clicking
            var hoverRadius = isMobile ? 15 : 10;
        ...
        // ============================================================
        // FAVORITES SYSTEM (LocalStorage + Export/Import)
        // ============================================================
        ...
        var FavoritesManager = {{
            ...
            remove: function(hutId) {{
                var favorites = this.getAll();
                var filtered = favorites.filter(function(id) {{ return id !== hutId; }});
            ...
            toggle: function(hutId) {{
                return this.has(hutId) ? this.remove(hutId) : this.add(hutId);
            }},
            ...
            exportJSON: function() {{
                ...
                var favoriteHuts = huts.filter(function(hut) {{
                    return favoriteIds.includes(hut.lat + '_' + hut.lon);
                }});
                ...
                var exportData = {{
                    ...
                    favorites: favoriteHuts.map(function(h) {{
                        return {{
                            id: h.lat + '_' + h.lon,
                            name: h.name,
                            latitude: h.lat,
                ...
                input.onchange = function(e) {{
                    ...
                    reader.onload = function(e) {{
                        try {{
                            ...
                            var importCount = imported.favorites.length;
                            var currentCount = FavoritesManager.count();
                            var message = 'Import ' + importCount + ' favorites?\\n\\n';
                            if (currentCount > 0) {{
                                message += 'Current: ' + currentCount + ' favorites\\n';
                                message += 'Will merge (no duplicates)';
                            }}
                            
                            if (!confirm(message)) return;
                            
                            var currentFavs = FavoritesManager.getAll();
                            var newFavs = imported.favorites.map(function(fav) {{ return fav.id; }});
                            var merged = Array.from(new Set(currentFavs.concat(newFavs)));
        ...
        function getHutId(hut) {{ return hut.lat + '_' + hut.lon; }}
        function showToast(msg) {{
            var t = document.createElement('div');
            t.style.cssText = 'position: fixed; bottom: 80px; left: 50%; transform: translateX(-50%); background: #1e293b; color: white; padding: 14px 28px; border-radius: 10px; font-size: 14px; font-weight: 600; z-index: 10000; box-shadow: 0 6px 20px rgba(0,0,0,0.3);';
            t.textContent = msg;
        ...
        function updateFavoritesCount() {{
            ...
            if (counter) counter.textContent = FavoritesManager.count();
        }}
        ...
        // Show hut details in detail sidebar
        ...
        // Initial stats and favorites count
        updateStats();
        updateFavoritesCount();
        console.log('Map ready with ' + markers.length + ' markers!');
        console.log('Favorites system ready. Loaded ' + FavoritesManager.count() + ' saved huts.');
    ...
    <div class="footer-bar">
        ...
        <a href="about.html" target="_blank">About & Philosophy</a>
        <span class="footer-divider">|</span>
        <span>🌍 Open Source</span>
        <span class="footer-divider">|</span>
        <a href="https://github.com/barcarolol-bit/Mountain-huts-europe" target="_blank" rel="noopener">
    ...
    # Write file
    output_path = Path(__file__).parent.parent / "web" / "map.html"
