...
<body>
    ...
    <script>
        ...
        // Store markers and create marker cluster group with performance optimizations
        var markers = [];
        var markerCluster = L.markerClusterGroup({{
            ...
            chunkedLoading: true,
            chunkInterval: 200,  // Process 200 markers at a time
            chunkDelay: 50,      // 50ms between chunks
            removeOutsideVisibleBounds: true,  // Remove markers outside view for better performance
            animate: true,
            animateAddingMarkers: false,  // Disable animation for faster initial load
        ...
        huts.forEach(function(hut) {{
            ...
            // Weather Widget (will be loaded dynamically)
            popupParts.push('<div id="weather-' + hut.lat + '-' + hut.lon + '" style="margin: 12px 0; padding: 0;"></div>');
            
            // Nearby Huts Section (will be populated dynamically)
            popupParts.push('<div id="nearby-' + hut.lat + '-' + hut.lon + '" style="margin: 12px 0;"></div>');
            
            popupParts.push('</div>'); // End body
            
            // Footer section with source link and last updated
            popupParts.push('<div style="background: #f8fafc; padding: 14px 20px; border-top: 1px solid #e2e8f0;">');
            
            // Last Updated Indicator
            popupParts.push('<div style="display: flex; align-items: center; justify-content: center; gap: 6px; margin-bottom: 10px; font-size: 11px; color: #64748b;">');
            popupParts.push('<span>🕐</span> <span>Data from <strong>' + escapeHtml(hut.source) + '</strong></span>');
            popupParts.push('</div>');
            
            if (hut.url && hut.url !== 'N/A' && hut.url !== '' && hut.url !== 'http://www.mountainhuts.info/map') {{
                popupParts.push('<a href="' + escapeHtml(hut.url) + '" target="_blank" rel="noopener" style="display: flex; align-items: center; justify-content: center; gap: 8px; padding: 12px 20px; background: linear-gradient(135deg, #2563eb, #3b82f6); color: white; text-decoration: none; border-radius: 10px; font-weight: 700; font-size: 14px; box-shadow: 0 4px 12px rgba(37, 99, 235, 0.3); transition: all 0.3s ease; text-align: center;" onmouseover="this.style.transform=\\'translateY(-2px) scale(1.02)\\'; this.style.boxShadow=\\'0 6px 20px rgba(37, 99, 235, 0.4)\\';" onmouseout="this.style.transform=\\'translateY(0) scale(1)\\'; this.style.boxShadow=\\'0 4px 12px rgba(37, 99, 235, 0.3)\\';">📍 View Full Details <span style="font-size: 16px;">→</span></a>');
            }} else {{
                popupParts.push('<div style="text-align: center; font-size: 12px; color: #94a3b8; padding: 4px;">No source URL available</div>');
            }}
        ...
        function loadWeather(lat, lon) {{
            ...
            if (apiKey === 'YOUR_OPENWEATHERMAP_API_KEY') {{
                weatherDiv.innerHTML = '<div style="padding: 10px; background: #fef3c7; border-left: 3px solid #f59e0b; border-radius: 6px; font-size: 12px; color: #92400e;">🌤️ Weather: <a href="https://openweathermap.org/weathermap?basemap=map&cities=true&layer=temperature&lat=' + lat + '&lon=' + lon + '&zoom=10" target="_blank" style="color: #2563eb;">View Forecast →</a><br><small style="opacity: 0.7;">Add OpenWeatherMap API key for live weather</small></div>';
                return;
            }}
            
            weatherDiv.innerHTML = '<div style="padding: 8px; text-align: center; font-size: 12px; color: #94a3b8;">Loading weather...</div>';
            ...
            fetch('https://api.openweathermap.org/data/2.5/weather?lat=' + lat + '&lon=' + lon + '&units=metric&appid=' + apiKey)
                ...
                .then(data => {{
                    ...
                    var weatherHtml = '<div style="padding: 12px; background: linear-gradient(135deg, #e0f2fe, #bae6fd); border-radius: 8px; display: flex; align-items: center; gap: 10px; border: 1px solid #7dd3fc;">';
                    weatherHtml += '<img src="' + iconUrl + '" style="width: 50px; height: 50px; margin: -8px 0;">';
                    weatherHtml += '<div style="flex: 1;">';
                    weatherHtml += '<div style="font-size: 24px; font-weight: 700; color: #0c4a6e;">' + temp + '°C</div>';
                    weatherHtml += '<div style="font-size: 12px; color: #075985; text-transform: capitalize;">' + description + '</div>';
                    weatherHtml += '</div>';
                    weatherHtml += '<a href="https://openweathermap.org/city/' + data.id + '" target="_blank" style="font-size: 11px; color: #0284c7; text-decoration: none; font-weight: 600;">5-Day →</a>';
                    weatherHtml += '</div>';
        ...
        function loadNearbyHuts(lat, lon, currentHutName) {{
            ...
            // Show top 3 nearest
            var nearbyHtml = '<div style="padding: 10px; background: #fef3c7; border-left: 3px solid #f59e0b; border-radius: 6px; font-size: 12px;">';
            nearbyHtml += '<div style="font-weight: 700; margin-bottom: 8px; color: #92400e;">📍 ' + Math.min(nearbyHuts.length, 3) + ' Nearby Hut' + (nearbyHuts.length === 1 ? '' : 's') + '</div>';
            
            nearbyHuts.slice(0, 3).forEach(function(nearby) {{
                nearbyHtml += '<div style="padding: 6px 0; border-top: 1px solid rgba(146, 64, 14, 0.2); color: #78350f; cursor: pointer;" onclick="map.setView([' + nearby.lat + ', ' + nearby.lon + '], 14); map.closePopup();">';
                nearbyHtml += '<div style="font-weight: 600;">' + escapeHtml(nearby.name) + '</div>';
                nearbyHtml += '<div style="font-size: 11px; opacity: 0.8;">' + nearby.distance.toFixed(1) + ' km away';
            ...
            if (nearbyHuts.length > 3) {{
                nearbyHtml += '<div style="margin-top: 6px; font-size: 11px; opacity: 0.7; color: #92400e;">+ ' + (nearbyHuts.length - 3) + ' more within 10km</div>';
            }}
