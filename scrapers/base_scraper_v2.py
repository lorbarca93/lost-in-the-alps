...
class BaseScraperV2(ABC):
    ...
    def validate_hut_data(self, hut: Dict) -> tuple[bool, Optional[str]]:
        """
        ...
        """
        # Required fields
        if not hut.get('name') or not str(hut['name']).strip():
        ...
        if lat is not None:
            try:
                ...
                return False, f"Invalid latitude format: {lat}"
        ...
        if lon is not None:
            try:
                ...
                return False, f"Invalid longitude format: {lon}"
        ...
        if altitude is not None:
            try:
                ...
                if altitude < -500 or altitude > 9000:
                    self.logger.warning(f"Suspicious altitude: {altitude}m for {hut.get('name')}")
            except (ValueError, TypeError):
                pass  # Non-critical, just warn
    ...
    def save_huts_batch(self, huts: List[Dict], batch_size: int = 100):
        """
        Save huts in batches with validation
        ...
        for i in range(0, len(huts), batch_size):
            ...
            validated_batch = []
            for hut in batch:
                is_valid, error = self.validate_hut_data(hut)
                if is_valid:
                    validated_batch.append(hut)
                else:
                    skipped += 1
