...
from data_cleaner import clean_hut_data, validate_hut_data, standardize_hut_type
...
class RefugesInfoFastScraper:
        ...
        async with self.semaphore:  # Limit concurrent requests
            ...
            try:
                ...
                async with session.get(api_url, params=params, timeout=30) as response:
                    ...
                    # refuges.info returns: 'cabane non gardée', 'refuge gardé', 'gîte d'étape'
                    if 'refuge' in hut_type.lower() and 'gardé' in hut_type.lower():
                        hut_type_std = 'Staffed hut'
                    elif 'cabane' in hut_type.lower() and 'non gardé' in hut_type.lower():
                        hut_type_std = 'Unstaffed cabin'
                    elif 'gîte' in hut_type.lower():
                        hut_type_std = 'Guesthouse'
                    elif 'bivouac' in hut_type.lower():
                        hut_type_std = 'Bivouac'
                    elif 'abri' in hut_type.lower():
                        hut_type_std = 'Shelter'
                    else:
                        # Default to unstaffed cabin for unrecognized types
                        hut_type_std = 'Unstaffed cabin'
    ...
    async def process_batch(self, session, refuge_ids, batch_num, total_batches):
        ...
        # Filter out None and exceptions, clean and validate data
        valid_huts = []
        for result in results:
            if result and not isinstance(result, Exception):
                try:
                    # Clean the data
                    cleaned = clean_hut_data(result)
                    
                    # Validate
                    is_valid, error = validate_hut_data(cleaned)
                    if is_valid:
                        valid_huts.append(cleaned)
                self.successful += 1
                    else:
                        self.skipped += 1
                except Exception as e:
